...
function start() {
    read -p $'\e[1;92mEnter Username: \e[0m' user
    checkaccount=$(curl -L -s https://www.instagram.com/$user/ | grep -c "the page may have been removed")
    if [[ "$checkaccount" == 1 ]]; then
        printf "\e[1;91mInvalid Username! Please try again.\e[0m\n"
        sleep 1
        start
    else
        default_wl_pass="passwords.lst"
        read -p $'\e[1;92mPassword List (Press Enter for default list): \e[0m' wl_pass
...
command -v tor > /dev/null 2>&1 || { echo >&2 "I require tor but it's not installed. Run ./requriment.sh. Aborting."; exit 1; }
command -v curl > /dev/null 2>&1 || { echo >&2 "I require curl but it's not installed. Run ./requriment.sh. Aborting."; exit 1; }
command -v openssl > /dev/null 2>&1 || { echo >&2 "I require openssl but it's not installed. Run ./requriment.sh Aborting."; exit 1; }
